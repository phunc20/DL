...
# ╔═╡ c0c1262a-5e18-4609-9817-d5888e94fb53
begin
  import Pkg
  using Plots
  plotly()
...
# ╔═╡ 53d7d9db-4872-4a54-b5ed-0f1ca7b506d5
md"""
## Subgradient
I want a Neural Network (abbreviated NN from now on) whose loss function
I can visualize, so I come up with approximating $y = x^2$ using a single-layer NN:

Let $X, y, m$ be data, labels and the number of instances, respectively. Then our goal is to minimize the MSE (Mean Square Error)
```math
\newcommand{\norm}[1]{\lVert{#1}\rVert_{\ell^2}}
\begin{align}
  \text{loss}:\quad \mathbb{R}^2 &\to \mathbb{R}   \\
  (w, b) &\mapsto \frac{1}{m} \norm{\sigma(wX + b) - y}^2 \\
\end{align}
```

We choose this model because we humans normally could only visualize
spaces with dimension $\le 3$. By restricting to two unknowns,
$w \in \mathbb{R}$ and $b \in \mathbb{R}$, we can visualize the graph
of $L$
```math
\text{graph}(\text{loss}) = \left\{(w, b, \text{loss}(w,b)) \; \vert \; w, b \in \mathbb{R}\right\}
```

"""

# ╔═╡ ad8296af-2bbf-4265-af84-204b292ba215
begin
  model = Dense(1=>1, relu)
  println("model.weight = $(model.weight) (of shape $(size(model.weight)))")
  println("model.bias = $(model.bias) (of shape $(size(model.bias)))")
end

# ╔═╡ a8b782fc-3eb3-4d8a-9e23-fcaf4c157ce1
begin
...
# ╔═╡ 95bf162f-7f45-4f6f-b25b-10067f677929
begin
  train_ratio = 0.8
  n_train = round(Int, length(X) * train_ratio)
  X_train, X_test = X[1:n_train], X[n_train+1:end]
  length.([X_train, X_test])
end

# ╔═╡ f721810f-768b-4630-90bf-371953aa071d
begin
  f(x) = x^2
...
# ╔═╡ 854137b1-f2d1-44a2-8666-e7fc7b36ebc9
begin
  Plots.plot(X_train, y_train, seriestype=:scatter)
  Plots.plot!(X_test, y_test, seriestype=:scatter)
end

# ╔═╡ 6410bdb4-cbcd-4a4c-9abc-918286a37b9e
size(X_train), size(y_train)

# ╔═╡ 630085f4-d3ee-433e-9342-77186c697b8f
loss(w, b) = Flux.Losses.mse(relu.(w * X_train .+ b), y_train)

# ╔═╡ fdb37a3a-5740-4ed3-aa37-59fca2483f21
let
  ws = range(-10, 10, length=500)
  bs = range(-10, 10, length=500)
  zs = [loss(w, b) for w in ws, b in bs]
  #Plots.plot(ws, bs, zs, st=:surface)
  Plots.surface(ws, bs, zs)
end

# ╔═╡ a488df85-7a43-4fb2-8bb3-46a2153de342
md"""
My original intention was to visualize the point where the graph is not smooth (i.e. 
not differentiable), but (no matter how hard we rotate and zoom) it seems a little
...
md"""
## Convexity
Is convexity here important? I mean, if we only need non-linearity, could we use sth like
```math
\sigma(x) = \min\{x, 0\}
```
...
# ╔═╡ 0bba8945-a011-48e9-9c1f-ca085990c982
lulu(x) = min(x, 0)
...
It seems that most NNs are convex functions of input the weights and biases and of output the loss. [Recall that a composition of convex and affine functions is convex.]
...
# ╔═╡ 9560c013-fed4-4b78-8ea0-9fc1def61642
md"""
> Let's run an experiment of this using MNIST dataset!
>
> We shall train
> - One model with convex activation functions
> - One model with concave activation functions
"""
...
# ╔═╡ 33ed4357-bd2d-427d-aead-609dcb93e601
md"""
## Use Accuracy as A Performance Measure During Training
"""

# ╔═╡ e97fa5d2-068b-4633-bcbe-7a40cd62c1f1
#measures()
...
# ╔═╡ 5afce77c-9637-4f46-8e8e-7a484048e9b2
accuracy(convex_model(our_mnist["train"][1]), our_mnist["train"][2])

# ╔═╡ 3e08620f-3519-4482-98f4-f71d6ea53ef5
convex_model(our_mnist["train"][1])

# ╔═╡ 0cfb2c40-8043-4c3e-be0c-f49a52dcccbc
argmax(convex_model(our_mnist["train"][1]), dims=1)

# ╔═╡ 766b37e5-408e-4caf-acc1-713aaf31109d
accuracy([1, 3], [1, 2])

# ╔═╡ 7246b607-2f9c-4420-9343-0ce7a4e14c57
accuracy([[0.1, 0.01, 0.001], [1, 3, 15]], [1,2])

# ╔═╡ d710d588-6c46-4499-87ef-b9c9c0b11448
...
# ╔═╡ 53d33350-a8cf-4e92-b67c-18d10f8a6b50
acc(convex_model(our_mnist["test"][1]), our_mnist["test"][2])

# ╔═╡ 03538ce1-cf51-41dc-920e-51ce7e966e47
acc(concave_model(our_mnist["train"][1]), our_mnist["train"][2])

# ╔═╡ d1633ac7-69e7-441a-b9be-59ac691af8c5
acc(concave_model(our_mnist["test"][1]), our_mnist["test"][2])
...
git-tree-sha1 = "9e0309e1a1b40c2ee402264d8bee3bfee915c26b"
